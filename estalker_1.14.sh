...
echo -e "${CYAN}"
echo "#########################################################"
echo "#      Start downloading and installing  estalker       #"
echo "#########################################################"
echo -e "${NC}"

IPK_URL="https://raw.githubusercontent.com/Ham-ahmed/6-9/refs/heads/main/enigma2-plugin-extensions-estalker_1.14.20250920_all.ipk"
IPK_NAME="enigma2-plugin-extensions-estalker_1.14.20250920_all.ipk"
IPK_PATH="/var/volatile/tmp/$IPK_NAME"

echo -e "${YELLOW}Downloading the xklass...${NC}"
wget -O $IPK_PATH $IPK_URL

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ estalker has been downloaded successfully....${NC}"
else
    echo -e "${RED}✗ Failed to download estalker...${NC}"
    exit 1
fi

echo -e "${YELLOW}Downloading the estalker...${NC}"
