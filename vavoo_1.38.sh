...
echo -e "${CYAN}"
echo "#########################################################"
echo "#         Start downloading and installing  vavoo       #"
echo "#########################################################"
echo -e "${NC}"

IPK_URL="https://raw.githubusercontent.com/Ham-ahmed/6-9/refs/heads/main/enigma2-plugin-extensions-vavoo_1.38_all.ipk"
IPK_NAME="enigma2-plugin-extensions-vavoo_1.38_all.ipk"
IPK_PATH="/var/volatile/tmp/$IPK_NAME"

echo -e "${YELLOW}Downloading the vavoo...${NC}"
wget -O $IPK_PATH $IPK_URL

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ vavoo has been downloaded successfully....${NC}"
else
    echo -e "${RED}✗ Failed to download vavoo...${NC}"
    exit 1
fi
